...
output_log=open("ViViT_Deap_PAD.txt",'w')
...
# Load data
with open('Cropped_Face3_DEAP_s.pkl', 'rb') as file:
    data1 = torch.from_numpy(pickle.load(file)).float()
    print("Loaded 1st pickle")
with open('Cropped_Face4_DEAP_s.pkl', 'rb') as file:
    data2 = torch.from_numpy(pickle.load(file)).float()
    print("Loaded 2nd pickle")

final_data = torch.cat((data1, data2), dim=0)
final_data = final_data.permute(0, 3, 1, 2)  # Convert to (num_samples, channels, height, width)

with open('Cleaned_Labels.pkl', 'rb') as file:
    labels1 = torch.from_numpy(pickle.load(file)).float()
with open('Cleaned_Labels1.pkl', 'rb') as file:
    labels2 = torch.from_numpy(pickle.load(file)).float()

labels_d = torch.cat((labels1, labels2), dim=0)

with open('Cleaned_Labels_P.pkl', 'rb') as file:
    labels_p = torch.from_numpy(pickle.load(file)).float()

print("Loaded pleasure labels:", type(labels_p),labels_p.shape)

with open('Cleaned_Labels_A.pkl', 'rb') as file:
    labels_a = torch.from_numpy(pickle.load(file)).float()

print("Loaded arousal labels:", type(labels_a),labels_a.shape)


print("Loaded Labels Dominance", type(labels_d), labels_d.shape)
...
for unfrozen_layers in range(1,12):
    print("===================================================================================")
